...
- **👥 为团队而生** — 在整个团队内共享技能、知识库与 MCP 配置；每位成员仍保留自己的私有上下文
...
- **自动化** — 通过 cron 执行定时任务
- **团队协作** — 通过 S3 兼容对象存储同步团队知识库与文档；详见[团队协作](#团队协作)
...
团队共享的是若干专用目录，而非整个工作区。同步通过 S3 兼容对象存储进行（阿里云 OSS / WebDAV），没有 Git 模式。

同步由 `amuxd` 守护进程负责：每个团队在 `~/.amuxd/teams/<team_id>/` 下保留一份全局副本，每个已链接的工作区以软链接暴露被同步的目录。

### 共享内容

只有被同步的目录会离开本机，工作区其余内容保持本地：

- `team-knowledge/` — 团队知识库
- `team-documents/` — 团队文档（文件有归属人，可限制访问）

团队技能与团队 MCP 服务器不再走这棵树：技能来自技能注册表，MCP 服务器与团队环境变量来自 Cloud API。

### 注意事项

- 同步在应用启动时运行，也可从侧边栏的团队共享栏手动触发。
- 冲突同样出现在那里，在主面板中解决。
