...
export function makeOrgRolesRepo(host: OrgRolesHost) {
  ...
  return {
    ...
    /**
     * Contacts "set / remove admin" shortcut. Authz SoT is roles_users
     * (putMemberRoles); team_members.role is only the dual-write that
     * putMemberRoles already does. Writing the legacy column alone leaves
     * the list and 编辑角色 still showing 管理员.
     */
    async setTeamMemberRole(
      ...
      const caller = await host.resolveCallerActorForTeam(teamId);
      if (!caller) throw new ApiError(403, "forbidden", "not a member of this team");
      if (caller.id === actorId) {
        throw new ApiError(403, "forbidden", "cannot change your own role");
      }
      ...
      if (!userId) {
        throw new ApiError(
          400,
          "validation_failed",
          "actor has no user_id (agents/external cannot hold org roles)",
        );
      }
      const orgId = await resolveOrgId(teamId);
      const current = await listActiveMemberRoles(orgId, userId);
      if (current.some((r) => r.code === "owner")) {
        throw new ApiError(403, "forbidden", "cannot change the owner role");
      }

      const { data: catalog, error: catErr } = await publicFrom(host.supabase, "roles")
        .select("id, code, name, status")
        .eq("org_id", orgId);
      if (catErr) throw catErr;
      const rows = catalog ?? [];
      const adminRole = rows.find((r: { code: string; status: string }) =>
        r.code === "admin" && r.status === "active",
      );
      if (!adminRole?.id) {
        throw new ApiError(500, "internal_error", "org admin role is not defined");
      }

      const nextIds = current.map((r) => r.id).filter((id) => id !== adminRole.id);
      if (role === "admin") nextIds.push(adminRole.id);
      if (role === "member" && nextIds.length === 0) {
        const memberRole = rows.find((r: { code: string; status: string }) =>
          r.code === "member" && r.status === "active",
        );
        if (memberRole?.id) nextIds.push(memberRole.id);
      }

      await this.putMemberRoles(teamId, actorId, nextIds);
    },
  };
}
