...
describe("makeOrgRolesRepo", () => {
  ...
  test("setTeamMemberRole grants org admin and dual-writes team_members.role", async () => {
    const host = makeStubHost({
      teamRole: "owner",
      roles: systemCatalog,
      bindings: [
    ...
    const repo = makeOrgRolesRepo(host);
    await repo.setTeamMemberRole(TEAM, ACTOR, "admin");
    assert.deepEqual(
      host._bindings.map((b) => b.role_id).sort(),
      [ADMIN_ROLE, MEMBER_ROLE].sort(),
    );
    assert.equal(host._teamMembers[0].role, "admin");
  });

  test("setTeamMemberRole revokes org admin and keeps other roles", async () => {
    const host = makeStubHost({
      teamRole: "owner",
      roles: systemCatalog,
      bindings: [
    ...
    host._roles.push({ ...seededCustom });
    host._teamMembers[0].role = "admin";
    const repo = makeOrgRolesRepo(host);
    await repo.setTeamMemberRole(TEAM, ACTOR, "member");
    assert.deepEqual(
      host._bindings.map((b) => b.role_id).sort(),
      [CUSTOM_ROLE, MEMBER_ROLE].sort(),
    );
    assert.equal(host._teamMembers[0].role, "member");
  });

  test("setTeamMemberRole refuses to change an owner", async () => {
    const host = makeStubHost({
      teamRole: "owner",
      roles: systemCatalog,
      bindings: [
    ...
    const repo = makeOrgRolesRepo(host);
    await assert.rejects(
      () => repo.setTeamMemberRole(TEAM, ACTOR, "member"),
      (err: unknown) => {
        assert.ok(err instanceof ApiError);
        assert.equal(err.statusCode, 403);
        assert.match(err.message, /owner/i);
  ...
  test("setTeamMemberRole refuses to change the caller", async () => {
    const host = makeStubHost({
      teamRole: "owner",
      roles: systemCatalog,
      bindings: [
    ...
    const repo = makeOrgRolesRepo(host);
    await assert.rejects(
      () => repo.setTeamMemberRole(TEAM, "actor-1", "admin"),
      (err: unknown) => {
        assert.ok(err instanceof ApiError);
        assert.equal(err.statusCode, 403);
        assert.match(err.message, /own role/i);
        return true;
      },
    );
  });
