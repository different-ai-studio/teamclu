-- Atomic team skill version publish for the Supabase repository path.
-- Mirrors pg-repo createTeamSkillVersion: one transaction, row lock, CAS on
-- latest_version, optional marketplace detach, version insert, registry update.
...
begin
  ...
  v_when_to_use := coalesce(nullif(trim(p_when_to_use), ''), v_skill.when_to_use);
  v_when_not_to_use := coalesce(nullif(trim(p_when_not_to_use), ''), v_skill.when_not_to_use);
