...
**决定：每个团队创建时一次性 grant 10,000,000 credits。**

额度用 **credits 表述，不用元**。自主定价（§4.4）之后 credits 不再锚定成本，一笔额度值多少钱取决于售价，而售价是充值那一侧的运营参数 —— 用元描述赠送额会在调价当天变成错误陈述。

按 `catalog.example.yaml` 的占位价，10,000,000 credits ≈ **36 次典型 agent 会话**（标准档，单次累计约 20 万输入 + 2 万输出 token）。真实数字随定价决策变化，所以这个换算写在这里是给人估量级的，**不要拿它当合同**。
...
| 建团队一次性 grant 10,000,000 credits，不按人头 | 去掉 onboarding 门槛同时封住上限；按人头可靠拉人无限刷。用 credits 而非元表述——自主定价后元值取决于售价（§4.8） |
