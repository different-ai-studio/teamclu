...
13. ~~定期扫描 + `storage_bytes` 回写~~ —— 改为按需测量，理由见 §13.2
14. 超配额时拒发写凭证（上传签名与 STS 两处都拦）
...
§5 与 §11 阶段三说的是"定期 `ListObjectsV2` 扫描 + 回写"。实现改成了**在两个真正
做判断的地方按需测量**：签发上传 URL 和签发 STS 凭证。存的数字超过 15 分钟就先重新
测一次，再判断配额；没有配额在生效时一次都不测。

理由：定时任务要先存在。这个部署的 FC cron 是关着的（另一处还有 `search_path` 的
历史故障），两个部署目标都要各自接一遍；而且即使接上，最后一次扫描到这次请求之间
仍然有窗口。按需测量把那个窗口关掉了，还不需要任何基础设施。

代价是首次或长期空闲后的一次上传会多一次 List 调用。这个取舍写在
`usageForQuotaDecision` 的注释里。

**没有变的是语义**：用量依旧是最终一致的，`storage_bytes` 依旧不能拿来计费——
按需测量只发生在配额判断路径上，控制面读到的数字仍然可能是旧的。

### 13.3 多了一个 `auth: "app-token"` 路由模式
