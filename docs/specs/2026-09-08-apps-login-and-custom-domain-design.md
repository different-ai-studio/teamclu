...
- 改 `deploy/belayo/cloud-api.env.keys` — 与 self-host allowlist 保持同一批变量
...
5. ~~**批次 5 — 前端与模板**~~ ✅ **已完成**（部署配置已在批次 2/4 一并落地）。
   新增 `AppAuthSection` / `AppCustomDomainSection` 两个子组件、store 四个 action、
   28 个 locale key（中英）、三个模板的登录说明重写。前端全量 3393 绿。

   **实现时多做的三件事**：
   - **绑定同名域名改成幂等**（后端）：原本每次 PUT 都换 token，导致刷新页面后拿不回
     DNS 记录 —— 而重新 PUT 会让用户已经加好的 TXT 失效。现在同名保留 token 与校验状态，
     换名才重置。这是「查看要添加的记录」按钮能安全存在的前提。
   - **组件对旧服务端容错**：桌面端可能比 API 新，旧服务端的行里根本没有 `authRules`，
     读 `.length` 会让整个控制面白屏。缺字段一律回落到**严格值**（`org` / `all` / `[]`），
     顺带保证未知服务端不会让 app 看起来比实际更开放。
   - **顺手删了 main 带来的 4 个 i18n 死键**（`teamShare.save` 与三个 `skillConflict*`）
     —— `#1313` 删代码时没删 key，卡着同一个守卫，与本功能无关。

   **`authModePendingRedeploy` 的文案改了**：它以前说「站点仍然公开」，现在那是**假的**
   —— 墙立即生效，滞后的只是函数 env。改成「登录设置已生效，应用代码要读登录用户还需
   重新部署一次」。

   **仍未做**：真机验证。全部是单元测试，线上一次没跑过。
