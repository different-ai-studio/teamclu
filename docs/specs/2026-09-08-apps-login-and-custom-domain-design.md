...
`deployed_auth_mode` 的 pending 机制不需要跟着改：受众判定发生在**代理层**，不烘进函数
env，所以改受众**立即生效、不需要重新部署**。这是 D1 白送的一个好处，UI 上要说明
（登录方式的改动仍然需要重新部署，两者提示不同）。
...
登录方式那一块加受众选择器（「任何登录用户」/「本公司员工」），并说明**受众改动立即
生效、登录方式改动需要重新部署**（§4.8）。`teams.oid` 为空时禁用「本公司员工」并给出
原因。
...
3. **批次 3 — app 域名网关**：`apps-auth-gate.ts` + `app-auth-mode.ts` 改造 +
   ...
   换 app 后 cookie 失效。
4. **批次 4 — 自定义域名**：域名三列 + 三个路由 + lookup/ask 放宽 + 契约测试。
   验收：`ask` 对未校验域名答 404、对已校验域名答 200。
