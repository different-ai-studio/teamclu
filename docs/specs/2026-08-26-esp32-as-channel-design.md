...
计划 §9 的预算是 600–1100ms。2026-08-25 实测**首帧音频 2.2 秒**,已经超了,而
收编后 turn 之前还要多走:`dedup.claim`(本地)、`identity.actor_for`、
`router.resolve`(可能打云端)、`writer.write_inbound`(**一次云端写入**)。

`write_inbound` 是最贵的一步,而且内核注释明确写了它必须在 turn 之前
("Write BEFORE the turn, always")—— 那条规则是为了修另一个 bug,不能为语音单独
破例。

**这是本文最大的未决问题**,见 §8.1。不要先假设它可以接受。
...
1. **延迟**(§6.2)。收编后首帧只会更晚。需要先量出 `write_inbound` 到底花多少
   毫秒,再决定:接受、把它与 turn 并发(改内核,要正面回答它当初为什么串行)、
   还是给 caps 加一个"延迟敏感"维度。**没量之前不要开工第 1 步。**
