...
**八个渠道已经全部收编**(#1084 / #933):WeCom、飞书、邮件、Discord、KOOK、
SeaTalk、微信各有一个 `impl ChannelDriver`,连第三套会话映射 `session.rs` 都删掉了。

**ESP32 是唯一的例外。**它从 `voice/mic` 到 agent 自建了一条链路
...
4. **ESP32 最不像 IM**。初稿把这条写成"插队",那已经过期了 —— 原 spec 的迁移顺序
   (WeCom → 飞书 → 邮件 → 其余)在 #1084 里跑完了,ESP32 不是抢在别人前面,而是
   最后一个。风险的实质仍在:它是唯一一个没有文本输入框、没有消息列表、用户拿
   按钮说话的渠道,会最先撞上内核里残留的 IM 假设。区别是这次撞到了只能自己修,
   后面没有渠道跟着受益。
