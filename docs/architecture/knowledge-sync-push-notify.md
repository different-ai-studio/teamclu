...
3. **debounce 1–2 秒**，再落到已有的 `dispatch.sync_team`。它自带 per-team 互斥
   锁，不会和定时器打架 —— 正在跑就跳过，这条已有逻辑不用改。

## 7. 合批：一次 tick 不能变成 200 条广播
...
阶段 1 完全无风险（发到一个没人听的主题），可以先上，用真实流量验证合批窗口够不够。
