...
interface AppDataBrowserDialogProps {
  open: boolean
...
export function AppDataBrowserDialog({
  open,
  onOpenChange,
  app,
  tables,
  canEdit,
  initialTable,
}: AppDataBrowserDialogProps) {
  const { t } = useTranslation()
  const [selected, setSelected] = React.useState<string | null>(initialTable ?? tables[0]?.name ?? null)
  ...
  const handleSave = async () => {
    ...
    if (!original) return
    // Only what actually changed. Sending every column would make an unrelated
    // trigger fire and would overwrite a value someone else edited meanwhile.
    ...
    try {
      ...
      // Replace with what came back, not with the draft: triggers and defaults
      // rewrite values, and showing the submitted one would be a lie.
      setRows((prev) =>
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="flex h-[80vh] max-w-[min(1100px,92vw)] flex-col gap-0 overflow-hidden p-0">
          <DialogHeader className="shrink-0 border-b border-border-soft px-4 py-3">
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              ...
              {page?.nextCursor && (
                <div className="shrink-0 border-t border-border-soft px-3 py-2">
                  {/* "Load more", not page numbers: there is no total count —
                      count(*) is a full scan on a production table. */}
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
