import type { DaemonWorkspace } from "@/lib/daemon/daemon-workspaces";
import type { SessionWorkspaceRow } from "@/lib/cache/local-cache";
...
async function buildViewerWorkspaceContext(
  ...
  const [{ getLocalDaemonActorId }, { listDaemonWorkspaces }, { getBackend }] =
    await Promise.all([
      import("@/lib/daemon/daemon-agent-admin"),
      import("@/lib/daemon/daemon-workspaces"),
      import("@/lib/backend"),
    ]);
