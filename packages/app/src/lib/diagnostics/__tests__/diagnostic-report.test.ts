...
vi.mock('@/lib/diagnostics/live-dedup-stats', () => ({
...
vi.mock('@/lib/mqtt/mqtt-diagnostics', () => ({
...
vi.mock('@/lib/telemetry/startup-perf', () => ({
  getStartupTimeline: vi.fn(() => [{ name: 'app:ready', t: 120 }]),
}))

vi.mock('@/lib/diagnostics/console-capture', () => ({
  getConsoleEntries: vi.fn(() => [{ ts: 1, level: 'info', message: 'test', args: [] }]),
}))

vi.mock('@/lib/agent/runtime-state-snapshot', () => ({
