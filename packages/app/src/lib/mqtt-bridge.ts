...
import type * as TauriBridge from './mqtt-bridge-tauri'
...
function impl(): Promise<PlatformBridge> {
  if (isTauri()) {
    tauriBridge ??= import('./mqtt-bridge-tauri')
    return tauriBridge
  }
  browserBridge ??= import('./mqtt-browser-bridge')
...
export const listenForDaemonLiveStatus: typeof TauriBridge.listenForDaemonLiveStatus = async (
  ...
  if (!isTauri()) return () => {}
  const bridge = await import('./mqtt-bridge-tauri')
