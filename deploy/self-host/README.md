...
| `bootstrap/gen-secrets.sh` | 从 `JWT_SECRET` 派生 `ANON_KEY` / `SERVICE_ROLE_KEY` / `MQTT_SERVICE_TOKEN`；校验 `POSTGRES_PASSWORD`；`DASHBOARD_PASSWORD` 为空或仍是上游公开的占位值时生成随机值（已设置的不动）；根据 `CADDY_TLS_MODE` 写入 Caddy 变量；创建 `volumes` → `supabase/volumes` 软链 |
...
| `STUDIO_DOMAIN` | **yes** | Public domain for Supabase Studio. Proxied through Kong, never straight to the container: Studio has no login of its own |
| `DASHBOARD_USERNAME` / `DASHBOARD_PASSWORD` | auto | Studio login (Kong basic-auth), on `STUDIO_DOMAIN` and on `SUPABASE_DOMAIN`'s root. Password generated by `gen-secrets.sh` when blank; restart `kong` after changing it |
