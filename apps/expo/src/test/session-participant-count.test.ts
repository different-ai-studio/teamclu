...
describe("getSession participant count", () => {
  ...
  it("uses the server's count when it is present", async () => {
    const { sessions, urls } = api({ id: "s1", teamId: "t1", title: "x", mode: "collab", participantCount: 5 });
    const session = await sessions.getSession("t1", "s1");
    expect(session?.participantCount).toBe(5);
    expect(urls.some((url) => url.includes("/participants"))).toBe(false);
  });
});
